...
function(){console.log("Blockchain.info: Connection closed");$("#blockchainCheckBox").prop("checked")?StatusBox.reconnecting("blockchain"):StatusBox.closed("blockchain")};a.onerror=function(a){console.log("Blockchain.info: Connection Error: "+a)};a.onmessage=function(a){a=JSON.parse(a.data);if("utx"==a.op){for(var f=0,k=0;k<a.x.out.length;k++)f+=a.x.out[k].value;for(var g=f/satoshi,f=a.x.out,k=0;k<f.length;k++)if(f[k].addr==DONATION_ADDRESS){g=a.x.out[k].value/satoshi;new Transaction(g,!0);return}setTimeout(function(){new Transaction(g)},
Math.random()*DELAY_CAP)}else if("block"==a.op){var f=a.x.height,k=a.x.nTx,l=a.x.estimatedBTCSent;a=a.x.size;f>lastBlockHeight&&(lastBlockHeight=f,console.log("New Block"),new Block(f,k,l,a))}}}else console.log("No websocket support."),StatusBox.nosupport("blockchain")};TransactionSocket.close=function(){TransactionSocket.connection&&TransactionSocket.connection.close();StatusBox.closed("blockchain")};function TradeSocket(){}
